...
// WIN CHECK
// =========================
function checkWinner(game, setGame, setWinner, setRank) {
...
function drawCard(card) {
  ...
  if (card.shape === "circle") ctx.beginPath(), ctx.arc(cx, cy, 16, 0, Math.PI * 2), ctx.fill();
...
export default function WhotGame() {
  ...
  function playCard(i) {
    ...
    if (checkWinner(copy, setGame, setWinner, setRank)) {
      launchConfetti();
      return;
    }
    ...
    <div style={styles.bg}>
      ...
      {winner && (
        <div style={styles.overlay}>
          ...
          <p>Wins: {rank.wins} | Losses: {rank.losses} | Streak: {rank.streak}</p>
          <button style={styles.btn} onClick={rematch}>
            🔁 Rematch
          </button>
        </div>
      )}
...
const styles = {
  ...
  btn: { padding: 10, background: "#10b981", border: 0 },
  overlay: {
    position: "fixed",
    inset: 0,
